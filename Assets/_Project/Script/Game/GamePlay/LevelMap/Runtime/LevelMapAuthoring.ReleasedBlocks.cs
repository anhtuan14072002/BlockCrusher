...
public sealed partial class LevelMapAuthoring
{
    ...
    private Entity QueueReleasedBlockSpawn(Vector3 localPosition, Color32 color, ushort typeIndex, Vector3 sawCenter,
        ...
        float sideDamping, float maxVelocity, bool useColliderPhysics = false)
    {
        ...
        Vector3 releaseDirection = distance > 0.0001f ? outward / distance : Vector3.up;
        if (useColliderPhysics)
        {
            return CreateReleasedBlockEntity(position, color, typeIndex, Vector3.zero,
                safeMaxVelocity, false, false);
        }

        outward = releaseDirection;
